...
#Raster(height_pipe) + Raster(bnd_rast)
...
#segmented naip
  ...
  arcpy.Clip_analysis(sms_full, pipe_buffer, sms_fc)
  ...
  def classify(stage, landcover, field):
    if stage == "S1":
      if field == "S1_grid":
        ...
        #-----------------------------------------------
        # Thresholds
        imp = "-0.88 <= x <= -0.12" #[-0.88, -0.2]
        veg = "-0.01 <= x <= 0.6"  #[-0.18, 0.5]
        #-----------------------------------------------
        return ("def landcover(x):\\n"+
               "  membership = \"\"\\n"+
               "  if "+imp+":\\n"+
        ...
        #-----------------------------------------------
        # Thresholds
        imp = "-0.02 <= x <= 0.91"  #[0.24, 0.91]
        veg = "-0.46 <= x <= -0.03" #[-0.41, 0.18]
        #-----------------------------------------------
        return ("def landcover(x):\\n"+
               "  membership = \"\"\\n"+
               "  if "+imp+":\\n"+
        ...
        #-----------------------------------------------
        # Thresholds
        imp = "-0.94<= x <= -0.05"  #[-0.94, -0.17]
        veg = "-0.02 <= x <= 0.38" #[-0.3, 0.38]
        #-----------------------------------------------
        return ("def landcover(x):\\n"+
               "  membership = \"\"\\n"+
               "  if "+imp+":\\n"+
        ...
        #-----------------------------------------------
        # Thresholds
        imp = "-0.94 <= x <= -0.13"  #[-0.94, -0.25]
        veg = "-0.08 <= x <= 0.76" #[-0.15, 0.76]
        #-----------------------------------------------
        return ("def landcover(x):\\n"+
               "  membership = \"\"\\n"+
               "  if "+imp+":\\n"+
...
def convertToAscii(x):
  ...
  for layer in landscape_lst:
    ...
    ready = ExtractByMask(final, pipe_buffer)
  ...
  for layer in elevation_lst:
    ...
    temp = os.path.join(scratchgdb, "t_" + layer)
    if layer == "slope":
      arcpy.Slope_3d(scaled_dem, temp, "DEGREE")
    elif layer == "aspect":
      arcpy.Aspect_3d(scaled_dem, temp)
    elif layer == "elevation":
      temp = scaled_dem
    temp_raster = os.path.join(scratchgdb, "t_"+layer+"_r")
    arcpy.CopyRaster_management(temp, temp_raster, "", "", "0", "NONE", "NONE", "32_BIT_SIGNED","NONE", "NONE","GRID", "NONE")
    arcpy.DefineProjection_management(temp_raster,projection)
    ready = ExtractByMask(temp_raster, fuel)
    ready.save(temp_raster)
    ascii_layers.append([temp_raster, layer])
    arcpy.RasterToASCII_conversion(ready, ascii_output)
    arcpy.AddMessage("The {0} file was created.".format(layer))

convertToAscii(land_cover)
