...
# Projection options = ["UTMZ10", "UTMZ11", "SPIII", "SPIV"]
projection = "SPIII"
...
bnd_zones = os.path.join(inputs, "zones.shp")
...
def align_cells(raw_naip, raw_heights):
...
align_cells(raw_naip, raw_heights)
...
def zones(bnd_zones):
    ...
    while zones:
        ...
        zone_lst.append(zone_num)
    return zone_lst

zone_lst = zones(bnd_zones)
    
if __name__ == '__main__':
    pool = Pool(num_cores) #number of cores
    #-----------------------------------------------
    #-----------------------------------------------
    text = "Estimated time until complete: "+str(len(zone_lst)*0.5*num_cores)+"hr"
    generateMessage(text)
    #-----------------------------------------------
    
    pool.map(obfa, zone_lst)

def obfa(zone_num): #where zone_lst == ["0", "1", "2"]
    #-----------------------------------------------
    text = "Generating land cover objects for zone: "+str(zone_num)+"."
    generateMessage(text)
    #-----------------------------------------------
    #-----------------------------------------------
  ...
  #segmented naip
    ...
    #-----------------------------------------------
    #-----------------------------------------------
    text = "Zone: "+str(zone_num)+" complete."
    generateMessage(text)
    #-----------------------------------------------
