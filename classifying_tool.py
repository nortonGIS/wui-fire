...
sms_raster = os.path.join(scratchgdb, "temp_sms_raster")
...
sms = os.path.join(outputspath, "temp_sms.shp")
...
inZoneData = sms
zoneField = "ID"
outTable = os.path.join(scratchgdb, "temp_zonal_height")
inValueRaster = height
z_height = ZonalStatisticsAsTable(inZoneData, zoneField, inValueRaster, outTable, "NODATA", "MEAN")
arcpy.AddField_management(outTable, field, "FLOAT")
arcpy.CalculateField_management(outTable, field, "[MEAN]")
#arcpy.CalculateField_management(outTable, "ID", "[OBJECTID]")
zonal_tables.append((outTable, field))
arcpy.AddMessage("Height is created.")


# join zonal tables with SMS
sms_fc = os.path.join(scratchgdb, "temp_sms_fc")	#sms feature class
arcpy.FeatureClassToFeatureClass_conversion(sms, scratchgdb, "temp_sms_fc")
...
#arcpy.AddMessage("Composite created with {0}, {1}, {2}, and {3}.".format(layers[0], layers[1], layers[2], layers[3]))

# SVM
in_raster = composite
in_training_features = os.path.join(svmpath, "training_classes.shp")
classifier_definition = os.path.join(svmpath, "svm_class.ecd")
#max_samples = "150"
additional = out_rasterdataset
attributes = "COLOR;MEAN;STD;COUNT;COMPACTNESS;RECTANGULARITY"
#arcpy.gp.TrainSupportVectorMachineClassifier(in_raster, in_training_features, classifier_definition)
#arcpy.AddMessage("Classifier definition created.")
# Classiy Raster
svm_fuel_model = os.path.join(scratchgdb, "svm_fuel_model")
#classified = ClassifyRaster(in_raster, classifier_definition)
#classified.save(svm_fuel_model)

# Fuel Model
stage = "fuel_model"
landscape = os.path.join(svmpath, "landscape")
arcpy.RasterToPolygon_conversion(svm_fuel_model, landscape, "NO_SIMPLIFY", "CLASS_NAME")
landscape_fields = ["fuel", "canopy", "stand_ht"]
for field in landscape_fields:
	arcpy.AddField_management(landscape, field, "INTEGER")
	arcpy.CalculateField_management(outTable, field, "[MEAN]")
#arcpy.AddMessage("Site classified.")